...
export async function handler(chatUpdate) {
    ...
    try {
        ...
        try {
           ...
           let user = global.db.data.users[m.sender]
            ...
            if (settings) {
                ...
                if (!('bebot' in settings)) settings.bebot = false
        ...
        if (m.isBaileys)
            return      
          m.exp += Math.ceil(Math.random() * 10); // Ganar experiencia
        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                let metro = `${
          plugin.botAdmin || plugin.admin || plugin.group || plugin || noPrefix || hl || m.text.slice(0, 1) == hl || plugin.command
        }`;
        if (adminMode && !isOwner && !isROwner && m.isGroup && !isAdmin && metro) return;   
                ...
                if (plugin.premium && !isPrems) { // Users Premium
                    fail('premium', m, this)
                    continue
                }
                ...
                if (!isPrems && plugin.diamond && global.db.data.users[m.sender].diamond < plugin.diamond * 1) {
                    this.reply(m.chat, `💎 Tus diamantes se agotaron\nUsa este comando para comprar más diamantes\n\n*${usedPrefix}buy* <monto> \n*${usedPrefix}buy all*`, m)
               ...
               try {
                    ...
                    if (m.diamond)
                        m.reply(`Has Utilizado *${+m.diamond}* 💎`)
                }
                break
            }
        }
        ...
        case 'remove':
            if (chat.welcome) {
                ...
                for (let user of participants) {
                    let pp = 'https://telegra.ph/file/dadf8d41f99c345d92390.jpg'
                    let ppgp = 'https://telegra.ph/file/dadf8d41f99c345d92390.jpg'
                    try {
                        pp = await this.profilePictureUrl(user, 'image')
                        ppgp = await this.profilePictureUrl(id, 'image')
                        } finally {
                        let about = (await this.fetchStatus(user).catch(console.error) || {}).status || '×'
                        text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Bienvenido, @user').replace('@group', await this.getName(id)).replace('@date', date).replace('@bio', about).replace('@time', time).replace('@desc', groupMetadata.desc?.toString() || 'Desconocido') :
                            (chat.sBye || this.bye || conn.bye || 'Adiós, @user')).replace('@user', '@' + user.split('@')[0]).replace('@date', date).replace('@bio', about).replace('@time', time)

                            let wel = API('fgmods', '/api/welcome2', {
                            ...
                            /*this.sendButton(id, text, msg.ig(), action === 'add' ? wel : lea, [
...
export async function groupsUpdate(groupsUpdate) {
    ...
    for (const groupUpdate of groupsUpdate) {
        ...
        if (groupUpdate.desc) text = (chats.sDesc || this.sDesc || conn.sDesc || 'La descripción cambio a\n@desc').replace('@desc', groupUpdate.desc)
        ...
        if (groupUpdate.revoke) text = (chats.sRevoke || this.sRevoke || conn.sRevoke || 'El enlace del grupo cambio a\n@revoke').replace('@revoke', groupUpdate.revoke)
        if (!text) continue
...
global.dfail = (type, m, conn) => {
    ...
    owner: '🤖 Este Comando es solo para el *Owner y Sub Bots*',
    mods: 'Esta Comando es solo para *Para moderadores del Bot*',
    prem: 'Este Comando es solo para usuarios *Premium*',
    ...
    unreg: '🖨️ Regístrate para usar este Comando Escribiendo:\n\n*/reg nombre.edad*\n\n📌Ejemplo: /reg phoenix.16',
    restrict: 'Esta Comando está *desabilitado*'
    }[type]
